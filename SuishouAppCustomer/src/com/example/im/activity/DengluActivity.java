...
public class DengluActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		no_login = (Button) findViewById(R.id.no_login);
		login = (Button ) findViewById(R.id.login);
		no_login.getBackground().setAlpha(150); 
		login.getBackground().setAlpha(150); 
		no_login.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				switch (v.getId()) {
				case R.id.no_login:
					Intent intent1 = new Intent(DengluActivity.this,
							MainActivity.class);
					intent1.putExtra("login_status", 0);
					startActivity(intent1);
					Toast.makeText(DengluActivity.this, "您尚未登录哦~",
							Toast.LENGTH_SHORT).show();

				}
			}
		});
		Button zhuce_2 = (Button) findViewById(R.id.zhuce_2);
		zhuce_2.getBackground().setAlpha(150);
		zhuce_2.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {
				switch (v.getId()) {
				case R.id.zhuce_2:
					Intent intent1 = new Intent(DengluActivity.this, reg.class);
					startActivity(intent1);
					break;
				}
			}
		...
		ThreadUtils.runInSubThread(new Runnable() {
			...
			public void run() {
				try {
					...
					conn = new QQConnection("192.168.1.107", 8090);// Socket
					conn.connect();// 建立连接
					// 建立连接之后，将监听器添加到连接里面
			...
			ThreadUtils.runInUiThread(new Runnable() {
				...
				public void run() {
					if (QQMessageType.MSG_TYPE_BUDDY_LIST.equals(msg.type)) {
						...
						Intent intent1 = new Intent(DengluActivity.this,
								MainActivity.class);
						intent1.putExtra("login_status", 1);// 传递标志位
						startActivity(intent1);
						imapp.setFlag(1);
						finish();
